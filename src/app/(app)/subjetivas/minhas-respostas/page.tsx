...
export const metadata: Metadata = {
  title: "Minhas respostas subjetivas",
};
...
export default async function SubjectiveHistoryPage({
  searchParams,
}: PageProps) {
  const userId = await requireUserId();
  const params = await searchParams;
  const data = await getStudentManualAttempts(userId);
  const attempts = data.attempts.filter((attempt) => attempt.kind === "subjective");
    ...
    <main className="px-4 py-6 sm:px-6 lg:px-8">
      <section className="rounded-md border border-border-soft bg-surface p-5 sm:p-6">
        ...
        <p className="mt-3 text-sm leading-6 text-muted">
          Acompanhe envios, notas e feedbacks manuais.
        </p>
      </section>
      {params?.success ? (

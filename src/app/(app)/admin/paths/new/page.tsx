import type { Metadata } from "next";

import {
  AdminHeader,
  AdminNav,
  StatusMessages,
} from "@/components/admin/admin-ui";
import { PathForm } from "@/components/admin/path-form";
import { requireAdminPage } from "@/lib/admin/guard";

export const metadata: Metadata = {
  title: "Admin - Nova trilha",
};

type PageProps = {
  searchParams?: Promise<{ error?: string; success?: string }>;
};

export default async function NewPathPage({ searchParams }: PageProps) {
  await requireAdminPage();
  const params = await searchParams;

  return (
    <main className="px-4 py-6 sm:px-6 lg:px-8">
      <AdminHeader
        title="Nova trilha"
        description="Cadastre a trilha primeiro. Depois edite para adicionar e ordenar itens."
        backHref="/admin/paths"
      />
      <AdminNav />
      <StatusMessages success={params?.success} error={params?.error} />
      <section className="mt-6 rounded-md border border-border-soft bg-surface p-5 sm:p-6">
        <PathForm returnTo="/admin/paths/new" />
      </section>
    </main>
  );
}
